...
import { FormControl, InputLabel, MenuItem, Select, Typography, keyframes } from '@mui/material';
...
export const Tablee = ({data}) => {
  ...
  useEffect(() => {
    ...
    const fetchAddresses = async () => {
      const addresses = await Promise.all(data.map(async (row) => {
        const address = await getAddressFromLatLng(row.latitude, row.longitude);
  ...
  //   //Count the number of 'Stopped' states
  //   const stoppedCount = MOCK_DATA.filter(row => row.state === 'Stopped').length;
  //   setVehicleStoppedCount(stoppedCount);

  //   //Count the number of 'Overspeed' states
  //   const overspeedCount = MOCK_DATA.filter(row => row.state === 'Overspeed').length;
  //   setVehicleOverspeedCount(overspeedCount);

  //   //Count the number of 'Idle' states
  //   const idleCount = MOCK_DATA.filter(row => row.state === 'Idle').length;
  //   setVehicleIdleCount(idleCount);

  //   //Count the number of 'Unreachable' states
  //   const unreachableCount = MOCK_DATA.filter(row => row.state === 'Unreachable').length;
  //   setVehicleUnreachableCount(unreachableCount);
  // }, [MOCK_DATA]);
  ...
  return (
    ...
    <div style={{ display: 'flex', gap: '16px' }}>
              <FormControl variant="outlined" fullWidth sx={{width:250,'& .MuiInputBase-root': { height: 40 }}}>
              ...
              <FormControl variant="outlined" fullWidth sx={{width:250,'& .MuiInputBase-root': { height: 40 }}}>
                <InputLabel id="groups-label-4" style={{alignItems:'center'}}>Groups</InputLabel>
                <Select
              ...
              <FormControl variant="outlined" fullWidth sx={{width:250,'& .MuiInputBase-root': { height: 40 }, marginBottom:2}}>
              ...
              {/* GoogleMaps */}
              <Googlemap lati={latitude[0]} longi={longitude[0]}/>
              <br/></div>
  <Cards vehicleRunningCount={vehicleRunningCount} vehicleStoppedCount={vehicleStoppedCount} vehicleOverspeedCount={vehicleOverspeedCount} vehicleIdleCount={vehicleIdleCount} vehicleUnreachableCount={vehicleUnreachableCount}/><br/>
...
>
                    ...
                    >
                      ...
                      {columns.map((column) => (
                        column.accessor !== 'select' && columnVisibility[column.accessor] && (
                          ...
                          style={{
                            borderRight: '1px solid #ddd',
                            cursor: column.accessor === 'location' ? 'pointer' : 'default' // Add cursor style for indicating clickable element
                          }}
                          onClick={() => {
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={6}>
          <Paper><h3>Current Location</h3><br/>
          </Paper>
        </Grid>
        <Grid item xs={6}>
        <Paper><h3>Distance Travel(Today)</h3><br/>
        </Paper>
        </Grid>
        <Grid item xs={6}>
        <Paper><h3>Max Speed</h3><br/>
        </Paper>
        </Grid>
        <Grid item xs={6}>
        <Paper><h3>Distance Graph(Last 7 Days)</h3><br/>
        </Paper>
        </Grid>
        <Grid item xs={6}>
        <Paper><h3>Speed Graph(Last 3 hours)</h3><br/>
        </Paper>
        </Grid>
        <Grid item xs={6}>
        <Paper><h3>Landmark Report(Last 10 Records)</h3><br/>
        </Paper>
        </Grid>
        <Grid item xs={6}>
        <Paper><h3>Distance Location(Last 10 Records)</h3><br/>
        </Paper>
        </Grid>
        <Grid item xs={6}>
        <Paper><h3>All Points(Last 20 Records)</h3><br/>
        </Paper>
        </Grid>
        <Grid item xs={6}>
        <Paper><h3>Stop Report(Last 10 Records)</h3><br/>
        </Paper>
        </Grid>
        <Grid item xs={6}>
        <Paper><h3>Area In/Out Report(Last 10 Records)</h3><br/>
        </Paper>
        </Grid>
      </Grid>
    </Box>
